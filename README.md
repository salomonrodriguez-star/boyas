# Landing Boyas - Tienda SEMEX

Landing page de alta conversión para boyas viales. Stack: HTML/CSS/JS estático, desplegable en Vercel, formulario conectado a Google Sheets + apertura automática de WhatsApp.
...
├── styles.css              ← Estilos (paleta SEMEX)
├── script.js               ← Lógica del formulario + WhatsApp
...
git init
git add .
git commit -m "Initial commit - Landing Boyas"
git branch -M main
git remote add origin https://github.com/TU_USUARIO/landing-boyas.git
...
3. Importa el repositorio `landing-boyas`.
4. **Framework Preset:** "Other" (es HTML estático).
5. **Root Directory:** dejar en blanco (raíz).
6. Click en **Deploy**.

Vercel te dará una URL tipo `landing-boyas.vercel.app`. Para usar dominio propio (ej. `boyas.semex.com.mx`), entra a Settings > Domains.

### 3. Conectar Google Sheets

1. Crea un Google Sheet nuevo. Llámalo "Cotizaciones Boyas".
...
4. Guarda (Ctrl+S). Nombra el proyecto "Boyas Form Handler".
...
- **Producto:** Boya lisa / Boya con 1 reflejante
...
- Para modificar campos del formulario: edita el HTML, ajusta `construirMensajeWhatsApp()` en `script.js` y actualiza encabezados en `google-apps-script.gs`.
